...
void moveProductToDeposit(ArrayList* depositOriginList, ArrayList* depositDestinoList, Product* productToMove){
    ...
    int indexOfProductToMove, sizeDepositDestinoList, idOfLastProductDepositList, productMovedNewId;
    Product *productAux;
    Product *lastProductDepositDestList;
    ...
    //Obtengo la longitud del deposito destino
    sizeDepositDestinoList = depositDestinoList->len(depositDestinoList); 
    //Obtengo el ultimo elemento del deposito destino
    lastProductDepositDestList = depositDestinoList->get(depositDestinoList, sizeDepositDestinoList - 1);
    //Obtengo el id del ultimo elemento del deposito destino
    idOfLastProductDepositList = product_getId(lastProductDepositDestList);
    //Creamos un nuevo Id con respecto al Id del ultimo elemento del deposito destino
    productMovedNewId = idOfLastProductDepositList + 1;
    //Asignamos el nuevo Id al producto a ser trasladado
    product_setId(productAux,productMovedNewId,1);
    //Se agrega el producto a la lista destino
