...
int product_setId(Product* pProduct, int idProduct, int lowLimit){

    int returnAux = -1;

    if(idProduct >= lowLimit){

        pProduct->idProduct = idProduct;
        returnAux = 0;
    }
...
void product_print(Product* pProduct){
    ...
    printf("| Producto Id: %4d | Descripcion: %20s | Stock: %6d |\r\n",\
        pProduct->idProduct,pProduct->description,pProduct->stock);
}
